var background = {
    ...
    gameOver : function(){
        ...
        canvases.background.ctx.fillRect(0, window.innerHeight * 0.3 - gameoverImg.image.height, 
            window.innerWidth, window.innerHeight * 0.3 + gameoverImg.image.height);
        gameoverImg.draw(window.innerWidth / 2 - gameoverImg.image.width / 2,
            window.innerHeight * 0.2, null, null, canvases.background);
    },
    gameWin : function(){
        canvases.background.ctx.clear();
        canvases.background.ctx.globalAlpha = 0.9;
